...
SELECT cd.bookings.facid, name, SUM(slots) FROM cd.bookings
...
SELECT name, SUM(slots) FROM cd.bookings
LEFT JOIN cd.facilities
...
WHERE cd.facilities.facid ON (0,1)
AND starttime > '2012-09-21' AND starttime < '2012-09-22';

SELECT name, cast(starttime as time) FROM cd.bookings
LEFT JOIN cd.facilities
ON cd.bookings.facid = cd.facilities.facid
...
SELECT cast(starttime as time) FROM cd.bookings
LEFT JOIN cd.facilities
ON cd.bookings.facid = cd.facilities.facid
where memid = 28;